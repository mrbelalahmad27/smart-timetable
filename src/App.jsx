...
function App() {
    ...
    // Schedule native notifications when events change
    useEffect(() => {
        if (Capacitor.isNativePlatform()) {
            // Cancel all existing notifications first to avoid duplicates
            // Note: In a real app, you might want to be more selective
            // For now, we reschedule everything
                ...
                if (event.reminders && event.reminders.length > 0) {
                    ...
                    if (nextOccurrence) {
                        event.reminders.forEach(reminder => {
                            ...
                            const triggerTime = new Date(nextOccurrence.getTime() - offset * 60000);

                            if (triggerTime > new Date()) {
                                scheduleNotification(
                                    `${event.id}-${offset}`, // Unique ID
